...
class Planet(Entity):
    ...
    def has_docking_spots(self):
        """
        Determines if the planet has docking spots
...
class Ship(Entity):
    ...
    def navigate(self, target, game_map, speed, avoid_obstacles=True, max_corrections=90, angular_step=2.5,
    ...
    def new_navigate(self, target, game_map, speed, ship_grid, planet_grid, max_corrections=90, angular_step=2):
        if max_corrections <= 0:
            return None
        distance = self.calculate_distance_between(target)
        angle = self.calculate_angle_between(target)
        if game_map.close_obstacles_between(self, target, ship_grid, planet_grid, angle, distance, speed):
            new_target_dx = math.cos(math.radians(angle + angular_step)) * distance
            new_target_dy = math.sin(math.radians(angle + angular_step)) * distance
            new_target = Position(self.x + new_target_dx, self.y + new_target_dy)
