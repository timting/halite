...
class Map:
    ...
    def competitor_owned_planets(self, me):
        return [planet for planet in self.all_planets() if planet.is_owned() and planet.owner != me]

    def dockable_planets(self, me):
        return [planet for planet in self.all_planets() if not planet.is_owned() or (planet.is_owned() and planet.owner == me)]

    def unowned_planets(self):
        return [planet for planet in self.all_planets() if not planet.is_owned()]
    ...
    def close_obstacles_between(self, ship, target, ship_grid, planet_grid, angle, distance, speed):
        start_coords = [ship.y, ship.x]
        actual_distance = speed if distance > speed else distance
        new_target_dx = math.cos(math.radians(angle)) * actual_distance
        new_target_dy = math.sin(math.radians(angle)) * actual_distance
        end_coords = [ship.y + new_target_dy, ship.x + new_target_dx]
        # Get grid coordinates
        start_grid_coords = [int(np.ceil(coord / 15)) - 1 for coord in start_coords]
        end_grid_coords = [int(np.ceil(coord / 15)) - 1 for coord in end_coords]

        if start_grid_coords == end_grid_coords:
            entities = ship_grid[start_grid_coords[0]][start_grid_coords[1]] + planet_grid[start_grid_coords[0]][start_grid_coords[1]]
        else:
            entities = ship_grid[start_grid_coords[0]][start_grid_coords[1]] + planet_grid[start_grid_coords[0]][start_grid_coords[1]] + ship_grid[end_grid_coords[0]][end_grid_coords[1]] + planet_grid[end_grid_coords[0]][end_grid_coords[1]]
